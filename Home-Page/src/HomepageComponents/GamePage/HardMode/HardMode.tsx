...
const Medium = () => {
    ...
    let round = 0
    let score = 0
    let timer: number | undefined; // have to initialize is so i can make sure it is cleared below

    function nextround() {
        (document.getElementById("timer") as HTMLElement).style.display = "block"; // reset timer text
        (document.getElementById("map") as HTMLElement).style.display = "block"; // ensure map is visible
        if (round < 5) { 
    ...
    function initialize(){
        ...
        let time = 60; // one minute timer a round, starts at beginning of round
        ...
        timer = setInterval(function() {
            time--;
            timerElement.textContent = time + ' seconds remaining to make your guess';
            if (time <= 0){
                clearInterval(timer);
                timerElement.textContent = 'Times up! Click Continue to go to the next round.';
                buttonElement.style.display = "block"; //diplays continue
                buttonElement.onclick = function() {nextround();};
                (document.getElementById("map")  as HTMLElement).style.display = "none"; // gets rid of map so user cant guess
            }
        }, 1000);
        ...
        map.addListener("dblclick", (mapsMouseEvent: google.maps.MapMouseEvent) => {
            ...
            function getScore(){
                ...
                let total = Math.max(5000 - Math.floor(distance) * 5, 0); // score based on distance, linear
            ...
            if (round < 4){ // continue only shows for the first 4 rounds
                timerElement.style.display = "none"; //timer goes away
