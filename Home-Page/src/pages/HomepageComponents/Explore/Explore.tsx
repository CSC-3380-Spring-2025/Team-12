...
const Explore: React.FC<ExploreProps> = ({ isOpen, onClose }) => {
  ...
  // Array that contains interesting/fun facts for each location
  const funFacts = [
  ...
  // Set up a state to track the current fact
  const [currentFact, setCurrentFact] = useState<string>(getRandomFact());

  // Function for getting a random fact
  function getRandomFact() {
    const randomIndex = Math.floor(Math.random() * funFacts.length);
    return funFacts[randomIndex].fact;
  }

  // Logic for cycling to a new random fact
  const cycleFact = () => {
    ...
    <div className="modal-overlay">
      <div className="explore-modal">
        ...
        <div className="fun-fact">
            <h3>Fun Fact</h3>
          {/* Display the current random fun fact */}
          <p>{currentFact}</p>
        </div>

        {/* Button to cycle to new fact */}
        <button className="map-button" onClick={cycleFact}>New Fact</button>
